const fs = require("fs");
const extractMdData = require("extract-md-data");
...
const getNotes = (appDir, notesDir) => {
  const metaNotes = extractMdData(appDir, notesDir);

  const notes = metaNotes.map((mn) => {
    const { id, fm, content, relativePath: relPath, slug } = mn;
    const prettyUrl = getPrettyUrl(relPath);
    const rawUrl = getRawUrl(relPath);
    const lastNotedAgo = timeago.format(fm.lastNoted);
    return {
      id,
      fm,
      content,
